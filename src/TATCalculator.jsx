...
/* ════════════════ CALC — MODO DURACIÓN (SKY PREMIUM) ════════════════
   Una sola entrada editable: CM (hora de llegada).
   Todo lo demás se calcula automáticamente desde CM.
   Tabla: DURACIÓN | HORA | HITO
   DURACIÓN = delta de minutos entre hito actual y el anterior.
   HORA     = CM + planOff
══════════════════════════════════════════════════════════════════════ */
function CalcDur({ airlineKey, onLogout }) {
  const al = AL[airlineKey];
  if (!al) return null;
  const th = al.theme;
  ...
  /* ── Countdown hacia Entrega de vuelo ───────────────────────────── */
  const entSecs   = toMins(entRow.hora) * 60;
  const nowSec    = nowSecs();
  const secsLeft  = entSecs - nowSec;
  const cdMins2   = Math.floor(Math.abs(secsLeft) / 60);
  const cdSecs2   = Math.abs(secsLeft) % 60;
  ...
  /* ── Notificaciones ─────────────────────────────────────────────── */
  useEffect(() => { setupNotifications().then(ok => setNotifOk(ok)); }, []);
  ...
  /* ── Auto-abrir checklist cuando falta poco ─────────────────────── */
  useEffect(() => { if (cdUrgent && !showCL && !clComplete) setShowCL(true); }, [cdUrgent]);
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "10px 12px" }}>
        ...
        {history.map(h => (
          <div key={h.id} style={{ background: card, borderRadius: 12, padding: "12px 14px", marginBottom: 8, border: `1px solid ${bdr}` }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 6, marginBottom: 6 }}>
              {[["CM", h.cmReal], ["Entrega", addMins(h.cmReal, 28)], ["Push Back", h.pbReal]].map(([l, v]) => (
                <div key={l} style={{ textAlign: "center" }}><div style={{ fontSize: 7.5, fontWeight: 700, letterSpacing: "1px", textTransform: "uppercase", color: mut, marginBottom: 2 }}>{l}</div><div style={{ fontSize: 13, fontWeight: 800, color: txt, fontVariantNumeric: "tabular-nums" }}>{v}</div></div>
              ))}
            </div>
            {h.obs && <div style={{ fontSize: 10, color: mut, fontStyle: "italic", borderTop: `1px solid ${bdr}`, paddingTop: 5, marginTop: 4 }}>{h.obs}</div>}
    ...
    <div className="A" style={{ background: bg }}>
      ...
      {showCfg && (
        <div className="MOD" onClick={() => setShowCfg(false)}>
          <div className="MOD-box" style={{ background: card, border: `1px solid ${bdr}` }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 6, marginBottom: 16 }}>
              {[3, 5, 10, 15].map(m => (
                <button key={m} onClick={() => setAlertAt(m)} style={{ padding: "8px 0", borderRadius: 9, border: `1.5px solid ${alertAt === m ? th.accent : bdr}`, background: alertAt === m ? `${th.accent}18` : card, color: alertAt === m ? th.accent : mut, fontFamily: "'Plus Jakarta Sans',sans-serif", fontSize: 13, fontWeight: 700, cursor: "pointer" }}>{m} min</button>
              ))}
            </div>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              <span style={{ fontSize: 13, fontWeight: 600, color: txt }}>Modo nocturno</span>
      ...
      {showCL && (
        <div className="MOD" onClick={() => setShowCL(false)}>
          <div className="MOD-box" style={{ background: card, border: `1px solid ${bdr}` }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ fontSize: 11, color: mut, marginBottom: 10 }}>{clChecked}/{al.checklist.length} completados · PB a las {pbRow.hora}</div>
            <div style={{ width: "100%", height: 3, background: bdr, borderRadius: 3, marginBottom: 14, overflow: "hidden" }}>
              <div style={{ height: "100%", width: `${(clChecked / al.checklist.length) * 100}%`, background: clComplete ? "#4ADE80" : th.accent, borderRadius: 3, transition: "width .3s" }} />
            </div>
            {al.checklist.map((item, i) => (
      ...
      <nav className="N" style={{ background: th.nav }}>
        ...
        <div className="N-r">
          <button className="N-ico-btn" onClick={() => setShowCfg(true)} title="Configuración">{Ic.cfg}</button>
          <button className="N-ico-btn" onClick={() => setView("hist")} style={{ position: "relative" }} title="Historial">
            {Ic.hist}{history.length > 0 && <span className="N-badge">{history.length}</span>}
          </button>
      ...
      <div className="H" style={{ background: `linear-gradient(160deg,${th.gradA},${th.gradB})` }}>
        ...
        <div style={{ display: "flex", gap: 5 }}>
          {[
            { label: "Ent. Vuelo",    time: entRow.hora,         accent: false },
            { label: "Cierre puertas",time: cpRow?.hora,         accent: false },
            { label: "Push Back",     time: pbRow.hora,          accent: true  },
          ].map((a, i) => (
            <div key={i} style={{ flex: 1, background: "rgba(0,0,0,0.15)", borderRadius: 8, padding: "5px 6px", textAlign: "center" }}>
              <div style={{ fontSize: 7, fontWeight: 700, letterSpacing: "1px", textTransform: "uppercase", color: "rgba(255,255,255,0.28)", marginBottom: 2 }}>{a.label}</div>
              <div style={{ fontSize: 13, fontWeight: 800, color: a.accent ? th.accent : "rgba(255,255,255,0.5)", fontVariantNumeric: "tabular-nums" }}>{a.time || "--:--"}</div>
            </div>
          ))}
        </div>
      </div>
...
html{font-size:16px;}
body,#root{background:#03060F;color:#fff;font-family:'Plus Jakarta Sans',sans-serif;-webkit-font-smoothing:antialiased;overscroll-behavior:none;min-height:100vh;}
@keyframes spin{to{transform:rotate(360deg);}}
@keyframes rIn{from{opacity:0;transform:translateY(4px);}to{opacity:1;transform:translateY(0);}}

/* ══════════════ LOGIN ══════════════ */
.L{min-height:100dvh;background:#0A0F1D;display:flex;align-items:center;justify-content:center;padding:20px;color:#fff;}
.L-box{width:100%;max-width:360px;background:#131B2E;border:1px solid #1E293B;border-radius:20px;padding:32px 24px 24px;box-shadow:0 20px 40px rgba(0,0,0,0.5);}
...
/* APP */
.A{max-width:390px;margin:0 auto;min-height:100dvh;display:flex;flex-direction:column;transition:background .3s;}
.N{height:48px;display:flex;align-items:center;justify-content:space-between;padding:0 12px;flex-shrink:0;}
.N-l{display:flex;align-items:center;gap:7px;}
.N-nm{font-size:14px;font-weight:800;color:#fff;letter-spacing:-0.3px;}
.N-r{display:flex;align-items:center;gap:5px;}
.N-cd{font-size:10px;font-weight:700;letter-spacing:2px;}
.N-out{background:rgba(255,255,255,0.08);border:none;border-radius:7px;width:28px;height:28px;display:flex;align-items:center;justify-content:center;cursor:pointer;color:rgba(255,255,255,0.4);}
.N-ico-btn{background:rgba(255,255,255,0.06);border:none;border-radius:7px;width:28px;height:28px;display:flex;align-items:center;justify-content:center;cursor:pointer;color:rgba(255,255,255,0.5);position:relative;}
...
/* LIST */
.ML{flex:1;overflow-y:auto;-webkit-overflow-scrolling:touch;padding:4px 10px 2px;}
...
.MOD-box{width:100%;max-width:390px;border-radius:20px 20px 0 0;padding:20px 18px 32px;max-height:88vh;overflow-y:auto;}
...
/* BOTTOM */
.B{border-top:1px solid;padding:8px 10px 18px;display:flex;align-items:center;justify-content:space-between;gap:4px;position:sticky;bottom:0;z-index:10;box-shadow:0 -2px 14px rgba(0,0,0,0.1);}
.B-btn{display:flex;flex-direction:column;align-items:center;gap:2px;background:none;border:none;cursor:pointer;font-family:'Plus Jakarta Sans',sans-serif;font-size:9px;font-weight:600;padding:3px 6px;border-radius:8px;position:relative;white-space:nowrap;}
.B-btn:active{transform:scale(0.9);}
.B-fab{width:48px;height:48px;border-radius:50%;border:none;cursor:pointer;display:flex;align-items:center;justify-content:center;color:#fff;margin-top:-12px;box-shadow:0 5px 18px rgba(0,0,0,0.25);flex-shrink:0;}
...
.OV-img{max-width:86vw;max-height:60vh;object-fit:contain;border-radius:16px;box-shadow:0 24px 60px rgba(0,0,0,0.6);}
